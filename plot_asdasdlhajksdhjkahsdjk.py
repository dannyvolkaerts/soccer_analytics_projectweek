...
# Interpolate the ball data
ball_interp_x = interp1d(df_ball.index, df_ball['x'], kind='cubic')
ball_interp_y = interp1d(df_ball.index, df_ball['y'], kind='cubic')

# Interpolate the away players' data
away_interp_x = interp1d(df_away.index, df_away['x'], kind='cubic')
away_interp_y = interp1d(df_away.index, df_away['y'], kind='cubic')

# Interpolate the home players' data
home_interp_x = interp1d(df_home.index, df_home['x'], kind='cubic')
home_interp_y = interp1d(df_home.index, df_home['y'], kind='cubic')

# Generate a higher-resolution index for smoother transitions
interp_frames = np.linspace(0, len(df_ball) - 1, len(df_ball) * 5)

# Update the animate function to use interpolated data
def animate(i):
    """ Function to animate the data with easing."""
    # Get interpolated positions for the ball
    ball.set_data(ball_interp_x(interp_frames[i]), ball_interp_y(interp_frames[i]))
    
    # Get interpolated positions for away players
    away.set_data(away_interp_x(interp_frames[i]), away_interp_y(interp_frames[i]))
    
    # Get interpolated positions for home players
    home.set_data(home_interp_x(interp_frames[i]), home_interp_y(interp_frames[i]))
    
    return ball, away, home

# Call the animator with the new number of frames
anim = animation.FuncAnimation(fig, animate, frames=len(interp_frames), interval=10, blit=True)
plt.show()
